extends Node
class_name MemoryPatternManager
...
@export var score_goal_memory_game = 1
...
func player_clicked(colour):
	...
	if player_pattern.size() == pattern.size():
		...
		# Player wins after reaching a score of score_goal_memory_game
		if score >= score_goal_memory_game:
			win_game()
			return
